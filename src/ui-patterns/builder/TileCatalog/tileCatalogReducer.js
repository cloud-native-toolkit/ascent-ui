import { searchData } from 'carbon-addons-iot-react/lib/components/Table/tableReducer';
 ...
 *  page: PropTypes.number,
    ...
    selectedTileIds: PropTypes.array,
    // original tile data
  ...
  switch (action.type) {
    ...
    case TILE_ACTIONS.SELECT: {
      ...
      if (state.isMultiSelect) {
        ...
        if (selectedTileIds.includes(action.payload)) selectedTileIds.splice(selectedTileIds.indexOf(action.payload), 1);
        else selectedTileIds.push(action.payload);
        return {
          ...state,
          page,
